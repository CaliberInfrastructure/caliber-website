    ...
    <section className="py-24 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        <div className="text-center mb-20">
          <div className="inline-block px-4 py-1.5 mb-6 text-[10px] font-black tracking-[0.4em] text-cyan-400 uppercase bg-cyan-500/10 border border-cyan-500/20 rounded-full">
            The Process
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {STEPS.map((step, i) => (
            <div key={i} className="relative group">
              <div className="bg-white/5 backdrop-blur-sm rounded-[3rem] p-10 border border-white/10 hover:border-cyan-500/30 hover:bg-white/[0.08] transition h-full flex flex-col">
                {/* Step Number + Icon */}
                <div className="flex items-center justify-between mb-10">
                  <span className="text-7xl font-black text-white/[0.06] tracking-tighter select-none group-hover:text-cyan-500/10 transition">{step.number}</span>
                  <div className="w-14 h-14 bg-cyan-500/10 rounded-2xl border border-cyan-500/20 flex items-center justify-center group-hover:bg-cyan-500/20 transition">
                ...
                {/* Content */}
                <h3 className="text-2xl font-black text-white tracking-tight mb-4">{step.title}</h3>
                <p className="text-slate-400 text-sm leading-relaxed mb-8 flex-grow">{step.description}</p>

                {/* Bottom Tag */}
                <div className="pt-6 border-t border-white/10">
                  <span className="text-[10px] font-black text-cyan-400 uppercase tracking-widest">{step.detail}</span>
                </div>
              </div>

              {/* Connector Arrow (between cards, hidden on last) */}
              {i < STEPS.length - 1 && (
                <div className="hidden lg:flex absolute -right-4 top-1/2 -translate-y-1/2 z-10 w-8 h-8 bg-[#0B1221] border border-cyan-500/30 rounded-full items-center justify-center">
        ...
        <div className="mt-16 text-center">
          <a href="#audit" className="inline-flex items-center gap-3 bg-[#0097B2] text-white px-10 py-5 rounded-2xl text-lg font-black uppercase tracking-widest hover:bg-cyan-500 transition shadow-2xl shadow-cyan-950/50 group">
      ...
      <div className="absolute -top-24 -right-24 w-64 h-64 bg-cyan-500/5 blur-[80px] rounded-full"></div>
    </section>
  );
};
