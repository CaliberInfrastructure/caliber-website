          ...
          <div className="bg-white p-10 md:p-16 rounded-[4rem] shadow-2xl relative overflow-hidden">
            {submitted ? (
              <div className="text-center py-20 animate-in zoom-in duration-300">
                ...
                <h4 className="text-4xl font-black text-slate-900 mb-4 tracking-tighter">Audit Requested!</h4>
                <p className="text-slate-500 text-lg leading-relaxed">We'll reach out within 24 hours to begin your analysis.</p>
                <button onClick={() => setSubmitted(false)} className="mt-12 text-cyan-600 font-black hover:underline text-sm uppercase tracking-widest">
                  Send another request
                </button>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-6">
                <h3 className="text-4xl font-black text-slate-900 tracking-tighter mb-4">Request Your Free Audit</h3>
                <p className="text-slate-500 mb-10 text-sm font-medium">Takes 30 seconds. We'll reach out within 24 hours.</p>

                <div className="space-y-6">
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">Your Name *</label>
                    <input type="text" required value={formData.name} onChange={(e) => handleChange('name', e.target.value)} className="w-full p-6 rounded-2xl border-2 border-slate-50 bg-slate-50 font-bold text-slate-900 outline-none focus:border-cyan-500 focus:bg-white transition" />
                  </div>
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">Business Name *</label>
                    <input type="text" required value={formData.business} onChange={(e) => handleChange('business', e.target.value)} className="w-full p-6 rounded-2xl border-2 border-slate-50 bg-slate-50 font-bold text-slate-900 outline-none focus:border-cyan-500 focus:bg-white transition" />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">Phone *</label>
                      <input type="tel" required value={formData.phone} onChange={(e) => handleChange('phone', e.target.value)} className="w-full p-6 rounded-2xl border-2 border-slate-50 bg-slate-50 font-bold text-slate-900 outline-none focus:border-cyan-500 focus:bg-white transition" />
                    </div>
                    <div>
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">Email *</label>
                      <input type="email" required value={formData.email} onChange={(e) => handleChange('email', e.target.value)} className="w-full p-6 rounded-2xl border-2 border-slate-50 bg-slate-50 font-bold text-slate-900 outline-none focus:border-cyan-500 focus:bg-white transition" />
                    </div>
                  </div>
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">Monthly Revenue Range *</label>
                    <select required value={formData.revenueRange} onChange={(e) => handleChange('revenueRange', e.target.value)} className="w-full p-6 rounded-2xl border-2 border-slate-50 bg-slate-50 font-bold text-slate-900 outline-none focus:border-cyan-500 focus:bg-white transition appearance-none cursor-pointer">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">Avg. Treatment Price <span className="text-slate-300">(Optional)</span></label>
                      <div className="relative">
                        <span className="absolute left-5 top-6 text-slate-400 font-bold">$</span>
                        <input type="text" inputMode="numeric" pattern="[0-9]*" placeholder="450" value={formData.avgTreatmentPrice} onChange={(e) => handleChange('avgTreatmentPrice', e.target.value)} className="w-full p-6 pl-10 rounded-2xl border-2 border-slate-50 bg-slate-50 font-bold text-slate-900 outline-none focus:border-cyan-500 focus:bg-white transition" />
                      </div>
                    </div>
                    <div>
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">Missed Calls / Week <span className="text-slate-300">(Optional)</span></label>
                      <input type="text" inputMode="numeric" pattern="[0-9]*" placeholder="15" value={formData.missedCallsWeek} onChange={(e) => handleChange('missedCallsWeek', e.target.value)} className="w-full p-6 rounded-2xl border-2 border-slate-50 bg-slate-50 font-bold text-slate-900 outline-none focus:border-cyan-500 focus:bg-white transition" />
                    </div>
                    <div>
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">No-Show Rate <span className="text-slate-300">(Optional)</span></label>
                      <div className="relative">
                        <input type="text" inputMode="numeric" pattern="[0-9]*" placeholder="20" value={formData.noShowRate} onChange={(e) => handleChange('noShowRate', e.target.value)} className="w-full p-6 pr-10 rounded-2xl border-2 border-slate-50 bg-slate-50 font-bold text-slate-900 outline-none focus:border-cyan-500 focus:bg-white transition" />
                        <span className="absolute right-5 top-6 text-slate-400 font-bold">%</span>
                      </div>
                    </div>
                  </div>
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">Anything else we should know? <span className="text-slate-300">(Optional)</span></label>
                    <textarea rows={4} placeholder="e.g., biggest pain points, specific challenges..." value={formData.notes} onChange={(e) => handleChange('notes', e.target.value)} className="w-full p-6 rounded-2xl border-2 border-slate-50 bg-slate-50 font-bold text-slate-900 outline-none focus:border-cyan-500 focus:bg-white transition resize-none"></textarea>
                  </div>
                </div>
